...
interface WesCalendarResheduleOneDoneComponentProps extends GenericProps, RouteComponentProps<{id: number}> {
}

@transl()
@inject('rescheduleStore')
@observer
export default class WesCalendarResheduleOneDoneComponent extends React.Component<WesCalendarResheduleOneDoneComponentProps> {
        ...
        return (
            <ModalLayout show={this.showDialog} onClose={() => this.showDialog = false}>
                <div className="modal-header">
                    ...
                    {t('Done!')}
                    <p>{t('You sent a request to reschedule your lesson')} <span>{rescheduleStore.lesson.title}</span> <br /> from <span><WesDate date={rescheduleStore.lesson.date}  format="MMM DD, YYYY HH:mm" /></span> to <span><WesDate date={rescheduleStore.rescheduledDate} format="MMM DD, YYYY HH:mm" /></span>. <br />{t('As soon as your teacher replies, you will get a notification.')}</p>
                </div>
                <div className="modal-body">
                    <div className="text-center">
                        <Link to="../../../" className="btn btn-default" data-dismiss="modal">OK</Link>
                    </div>
                </div>
            </ModalLayout>
        );
